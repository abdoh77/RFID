...
int codeRead = 0; 
// String uidString; // veriable to show the UID of the scanned card.
...
String  passwd = "12345"; // Default password
...
void setup() {
  ...
  digitalWrite(lock,HIGH); // set the locker as locked
}
...
void readRFID() // check if the RFID card type is MIFARE Classic.
{
  ...
  if (piccType != MFRC522::PICC_TYPE_MIFARE_MINI &&  
    ...
    //printDec(rfid.uid.uidByte, rfid.uid.size); // Uncomment to see the UID for scanned cards

    //uidString = String(rfid.uid.uidByte[0])+" "+String(rfid.uid.uidByte[1])+" "+String(rfid.uid.uidByte[2])+ " "+String(rfid.uid.uidByte[3]); // To show the UID of RFID card
  ...
  // Serial.println(uidString); 
    ...
    if (passwd == passwd2){
      Serial.println("\nI know this card!"); // if authorized card scan with correct password entered it will print I know this card
      pinMode(lock, HIGH); // Do not unlocked. 
        ...
        { // check if password is correct
           ...
           // Serial.println("User pressed # I am going to check password"); // I have used this line just for debugging 
            ...
            if(passwd == passwd2)
            {
             ...
             Serial.println("  ");  
              ...
              digitalWrite(lock,LOW);//Unlock the locker.
          ...
          //    Serial.println(errmsg);
             ...
             Serial.println("   ");
              ...
              digitalWrite(lock,HIGH); // Do not Unlock the locker.
            }
            delay(60);      
         }           
    ...
    if (key == 'D'){  
      ...
      //  passwd = "";
        passwd2 = passwd;
         ...
         Serial.println("   ");
              Serial.println("Current password is wrong");
